...
/**
 * The deadline on the four Core 4 reads.
 *
 * FOUR SECONDS, NOT TWO. It was two — "the same deadline recall has had for
 * months" — and two seconds is not enough for four Supabase round trips from a
 * cold serverless function, where the first connection alone can take most of
 * it. Missing it is not free: the timeout reads as "the state could not be
 * read", which loses the carried-forward state, writes nothing that turn, and
 * (until the fix in webIntent) silently switched off every lookup as though the
 * conversation had been put off the record. A cold start was enough to do it.
 *
 * Doubling it costs at most two more seconds on the pathological turn, against a
 * reply that takes about eight anyway — and the original comment's own reasoning
 * points this way: a turn without its ledger has less continuity, a turn that
 * never arrives is an outage, and only one of those is on the table here.
 */
...
export async function prepareTurn(input: TurnInput): Promise<PreparedTurn> {
  ...
  // SAID OUT LOUD, because everything it costs is invisible from the outside:
  // no carried-forward state, nothing written this turn, and a reply that has to
  // work without the conversation's history. It is also the fault that spent
  // four rounds of production debugging disguised as a broken search, so the one
  // line that names it is worth the log.
  if (policyUnknown && canStore) {
    console.error('[core4] state read failed; this turn has no carried-forward state and writes nothing', {
      ms: ms.load,
      deadline: STORE_TIMEOUT_MS,
    });
  }
  ...
  const researching = runResearch({
    ...
    //
    // 'unknown' WHEN THE READ FAILED — WHICH IS NOT 'none'.
    //
    // This said 'none', fail-closed by analogy with persistence six lines up.
    // The analogy is wrong and it cost Core 4 its internet: 'none' means "they
    // asked for this to be off the record", so an unreadable row was read as an
    // instruction nobody gave. A missing table or an absent service key fails
    // every read on every turn, so this was not a bad second — it was the
    // feature gone, permanently, while Logos searched the same provider fine.
    //
    // A sensitive, unsafe or off-the-record MESSAGE is still refused: those come
    // from `signals`, which is computed from their words and needs no database.
    // See webIntent's `policy` parameter for the full reasoning.
    policy: policyUnknown ? 'unknown' : prior?.persistPolicy,
  ...
  const wantedWeb = webIntent(
    input.lastUserText,
    signals,
    policyUnknown ? 'unknown' : prior?.persistPolicy
  );
  // AND A LOOKUP REFUSED ON POLICY MUST SAY SO RATHER THAN SAY NOTHING.
  //
  // `webIntent` with no policy asks a narrower question: did THIS MESSAGE ask
  // for a lookup? When that is yes and the policy-aware answer is no, the gate
  // shut because of the conversation — off the record, or a subject that made it
  // conversation-only — and the old code emitted no block at all, leaving the
  // model to account for the missing sources itself. It said "I can't browse the
  // web in real time": a false sentence about Socria's nature, generated by a
  // privacy control working correctly. Nothing is sent either way; the
  // difference is whether the reply is honest about why.
  const refusedByPolicy =
    !wantedWeb.want && webIntent(input.lastUserText, signals).want ? wantedWeb.why : undefined;
  move += research
    ? renderResearch(research)
    : renderNoResearch(wantedWeb.want, buildQuery(input.lastUserText, []), refusedByPolicy);
