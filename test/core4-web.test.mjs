...
console.log('\n=== turns that must NOT reach the internet ===');
{
  ...
  // AND A DATABASE THAT COULD NOT BE READ IS NOT AN INSTRUCTION.
  //
  // The reported failure, in one assertion. The caller passed 'none' when the
  // state read failed, by analogy with persistence — where failing closed is
  // right. Here it meant an unreadable row was read as "they asked for this to
  // be off the record", and since a missing table or an absent service key
  // fails EVERY read on EVERY turn, Core 4 did not degrade: it lost its
  // internet permanently, on a deployment where Logos searched the same
  // provider fine. That is exactly the shape of what was reported.
  const unknown = webIntent('find me recent uta shorthorn articles on AI', NO, 'unknown');
  ok('an unreadable state row does not shut the gate', unknown.want === true, unknown.why);
  ok('  and the lookup is still recognised as one', unknown.kind === 'search');
  // What DOES still protect the turn: their own words, which need no database.
  ok('a sensitive message is refused with the row unreadable too',
    webIntent('what are the current options for this diagnosis', { sensitive: true }, 'unknown').want === false);
  ok('  as is an off-the-record one',
    webIntent('look up the current pricing', { offRecord: true }, 'unknown').want === false);
  ok('  and an unsafe one',
    webIntent('what do I do right now, is this an emergency', { safety: true }, 'unknown').want === false);
  // The whole point of a fourth value: 'unknown' and 'none' must not behave
  // alike, or the collapse comes back the next time somebody reads this code.
  ok('unknown and none are not the same thing',
    webIntent('look up the filing deadline', NO, 'unknown').want !==
      webIntent('look up the filing deadline', NO, 'none').want);
}

console.log('\n=== a refusal on policy says so, instead of saying nothing ===');
{
  // The other half of the same failure. A conversation that mentioned a
  // diagnosis, or that somebody put off the record, is conversation-only from
  // then on — correctly. But renderNoResearch emitted NOTHING for it, so the
  // model had to account for the absent sources itself and wrote "I can't browse
  // the web in real time": a false sentence about what Socria is, generated by a
  // privacy control doing its job. The search still does not happen.
  const refused = renderNoResearch(false, 'the filing deadline', 'this conversation is off the record');
  ok('a policy refusal produces a block at all', refused.length > 0);
  ok('  and says it was a decision, not an inability', /not a limit on what you are/.test(refused));
  ok('  forbidding the claim outright', /never say or imply you are unable to search/i.test(refused));
  ok('  carrying the actual reason', /off the record/.test(refused));
  ok('  and still asks for the useful half', /exact query to paste/.test(refused));
  // Unchanged: a turn nobody asked a lookup on gets nothing, and a wanted lookup
  // that simply found nothing gets the other block.
  ok('no lookup wanted and no refusal is still silence', renderNoResearch(false, 'x') === '');
  ok('a wanted lookup that found nothing is the other block',
    /The lookup did not run/.test(renderNoResearch(true, 'x')));
  ok('  and the two blocks are never both emitted',
    !/The lookup did not run/.test(refused));
}
